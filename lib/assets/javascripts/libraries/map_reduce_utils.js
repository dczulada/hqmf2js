...
root.map = function(record, population, denominator, numerator, exclusion, denexcep, msrpopl, msrpopl2, observ, occurrenceId, isContinuousVariable, stratification) {
  ...
  if (isContinuousVariable) {
    value = calculateCV(record, population, msrpopl, msrpopl2, observ, occurrenceId, value, stratification)
  } else {
...
root.calculateCV = function(record, population, msrpopl, msrpopl2, observ, occurrenceId, value, stratification) {
  finalSpecifics = {};
  ...
  if (ipp) {
    ...
    if (hqmf.SpecificsManager.validate(ipp)) {
      ...
      if (hqmf.SpecificsManager.validate(measurePopulation)) {
        ...
        if (msrpopl2(measurePopulation.specificContext)){
          value.MSRPOPL = hqmf.SpecificsManager.countUnique(occurrenceId, measurePopulation)
        } else {
          value.MSRPOPL = msrpopl2(measurePopulation.specificContext);
        }
